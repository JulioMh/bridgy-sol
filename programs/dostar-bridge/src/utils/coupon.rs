...
impl Coupon {
    pub fn verify(&self, serialized_data: &Vec<u8>, be: &[u8; 64]) -> Result<(), Errors> {
        ...
        if recovered_pubkey.0.ne(be) {
            msg!("Invalid coupon");
            return Err(Errors::InvalidCoupon);
        }
        msg!("Valid coupon");
        Ok(())
    }

    pub fn hash(&self, serialized_data: &Vec<u8>) -> [u8; HASH_BYTES] {
        let hash = {
